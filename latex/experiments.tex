...
\begin{center}
\subfloat[
	The architecture of our LeNet-5 contains three two one-dimensional convolutional layers and three fully-connected layer.
	Both convolutional layers have kernel size of 5 and are followed by a pooling layer.
	The first one has 32  and the second 48 channels.
	The two first fully-connected layer has output of size 100.
	All activation are ReLU except the last one,
...
\begin{center}
\subfloat[
	The training and validation loss are similar proving no overfitting.
...
\begin{center}
\subfloat[
	The original paper states that the classification loss and the CORAL loss should be almost the same at the end of training.
...
\subfloat[
	The CORAL statistic without enforcing the CORAL loss grow significantly in comparison to the scenario in the plot above
