...
and the most complete catalogue of QSOs has 42\,552 spectra.
...
We believe that LAMOST has good reasons for not including those spectra in the LAMOST DR5 catalogue.
...
because we were not able to trained it correctly
even though we did hyperparameter optimisation.
Table~\ref{summary} clearly shows that neural network based domain adaptation does cannot significantly improve performance in comparison to baseline
when applied to astonomical data.
We would except an increase in a metric of at least 5\% as in the original paper of the deep domain methods on standard academical datasets.
...
Domain adaptation did not succeed in spite of the fact
that the distributions of the source and target domain are different.
...
All three PCA, t-SNE and UMAP shows
that the data does not occupy a single cluster
but the SDSS spectra concentra in the center and LAMOST spectra on the edges
(see Figures~\ref{pca}, \ref{tsne} and \ref{umap}).

Also the deep domain adaptation method behaved correclty.
DDC kept the MMD between the source an target distributions low as shown in~Figure~\ref{ddc_mmds}.
Deep CORAL achived the same with the CORAL metric for distribution difference
(see Figure~\ref{deep_coral_training}),
and DRCN learnt the auxiliary reconstruction task
that is supposed to support domain adapataion
(see Figure~\ref{reconstruction}).
Moreover, even, hyperparameter optimisation cannot improve performance of the deep domain adaptation methods.

Having that the problem has to be in the data.
Therefore, we visualise the incorect classification of the baseline
because the error were almost the same for all the methods.
In Figure~\ref{src_fp} and Figure~\ref{src_fn}, we display source false positives and soure false negatives, respectively.
...
We believe the misclassification are evidence for our conclusion
...
(for example, missing measurements at certain wavelength, bad extraction from CCD chip). 
...
and the same applies to the Office dataset~\cite{saenko2010}.
Such well-formed dataset provide comfortable environment for basic research.
...
As we have shown astronomy provides such volume of data that is impossible to make clean.
...
Maybe, the imperfect data problem is the reaseon
why previous application of domain adaptation used active learning (a human expert)
after domain adaptation (see Section~\ref{da_astronomy}).

\begin{figure}
\includegraphics[width=\textwidth]{img/source_fp.pdf}
\caption{Source FP}
\label{src_fp}
\end{figure}

\begin{figure}
\includegraphics[width=\textwidth]{img/source_fn.pdf}
\caption{Source FN}
\label{src_fn}
\end{figure}

\begin{figure}
\includegraphics[width=\textwidth]{img/target_fp.pdf}
\caption{Target FP}
\label{src_fp}
\end{figure}

\begin{figure}
\includegraphics[width=\textwidth]{img/target_fn.pdf}
\caption{Target FN}
\label{src_fn}
\end{figure}
